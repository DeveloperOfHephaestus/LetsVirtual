// file responsible for loading About page 
...
const Home=Scene_Handler.Model_Adder("./assets/About.glb","h");
...
//model Adder
function AddModels()
{ Scene_Handler.Add_Recursive_Collection([Greeter,Home,Greeter2,Greeter3,documentsCase,BG]);
  //default greeter  
...
console.log(GreeterSelected);
  Scene_Handler.ChangePosition("h",-3,0,2);
...
if(GreeterSelected=="JSON")
{Animation_Handler.PlayAnimation("Ja4")}
if(GreeterSelected=="Armina")
{Animation_Handler.PlayAnimation("Aa4")}


//breif case
if(GreeterSelected=="Default")
{ Scene_Handler.Add_to_Scene("case");
  documentsCase.model.scale.set(26,26,26);
  Greeter.model.getObjectByName("LeftHand").add(documentsCase.model);
  documentsCase.model.quaternion.setFromAxisAngle(new THREE.Vector3(1,0,0),Math.PI/2);
  Scene_Handler.ChangePosition("case",0,0.25,0.05);

}
if(GreeterSelected=="JSON")
{ Scene_Handler.Add_to_Scene("case");
  documentsCase.model.scale.set(26,26,26);
  Greeter2.model.getObjectByName("LeftHand").add(documentsCase.model);
  documentsCase.model.quaternion.setFromAxisAngle(new THREE.Vector3(1,0,0),Math.PI/2);
  Scene_Handler.ChangePosition("case",0,0.25,0.05);
...
{ Scene_Handler.Add_to_Scene("case");
  documentsCase.model.scale.set(26,26,26);
  Greeter3.model.getObjectByName("LeftHand").add(documentsCase.model);
  documentsCase.model.quaternion.setFromAxisAngle(new THREE.Vector3(1,0,0),Math.PI/2);
  Scene_Handler.ChangePosition("case",0,0.25,0.05);
}
...
function RenderLoop(){
   ...
   if(Greeter.model&&Greeter2.model&&Greeter3.model&&Home.model&&documentsCase.model&&Animation_Handler)
   {rayCaster.setFromCamera(pointer,Camera);
    const rayCastIntersect=rayCaster.intersectObject(documentsCase.model);
     if(rayCastIntersect.length!==0)
     { beingIntersected=true;
      document.body.style.cursor="pointer";
     }
     else {beingIntersected=false;document.body.style.cursor="default";}

   }
   //about show
   if(beingIntersected==true&&isClicked==true)
   {document.getElementById("BreifCaseDiv").style.display="flow-root";


   if(Animation_Handler&&document.getElementById("BreifCaseDiv").style.display=="flow-root")
    {Animation_Handler.StopAnimation("Ja4");
    Animation_Handler.StopAnimation("Aa4");
    Animation_Handler.PlayAnimation("Ja5");
    Animation_Handler.PlayAnimation("Aa5");
    }
  
  }
  else if(Animation_Handler&&document.getElementById("BreifCaseDiv").style.display=="none")
  {Animation_Handler.StopAnimation("Ja5");
  Animation_Handler.StopAnimation("Aa5");
  Animation_Handler.PlayAnimation("Ja4");
  Animation_Handler.PlayAnimation("Aa4");

  }
  
  if(BG.model){BG.model.rotation.y+=0.001}
...
//breif case click handler
const BreifCaseDiv=document.getElementById("BreifCaseDiv");

if(isMobile==false)
BreifCaseDiv.style="left:35%;top:5%";
const BreifCaseCancelButton=document.getElementById("CancelAbout");
BreifCaseCancelButton.addEventListener("click",function(){
  BreifCaseDiv.style.display="none";
})
